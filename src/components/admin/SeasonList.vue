    ...
    >
      ...
      >
        <el-form-item label="运动会名称" prop="seasonName">
          <el-input
            v-model="addForm.seasonName"
            placeholder="第一届运动会"
          ></el-input>
        </el-form-item>
        <el-form-item label="运动会主题描述" prop="seasonTopicDesc">
          <el-input
            v-model="addForm.seasonTopicDesc"
            placeholder="友谊第一比赛第二"
            type="textarea"
          ></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    addSeason() {
      const _this = this;
      if (_this.addForm.seasonBeginTime >= _this.addForm.seasonEndTime) {
    ...
    editSeason() {
      if(this.editForm.seasonBeginTime>=this.editForm.seasonEndTime){
        return  this.$message.error("结束时间无法比开始时间早!");
      }
      this.editForm.seasonStatus=="关闭"?this.editForm.seasonStatus="0":"1";
